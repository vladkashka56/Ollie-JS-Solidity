...
library Delaunay {
    ...
    function triangulate(int256[2][] memory _vertices, uint256 _idx) internal pure returns (uint256, int256[6][] memory, uint256[] memory) {
        ...
        uint256[] memory edges_res = new uint256[](500);
        ...
        for (uint i = indices.length; i>_idx; i--) {
        ...
        return (open_len, open, edges_res);
    ...
    function drawTriangles_1(int256 x1, int256 y1, int256 x2, int256 y2, int256 x3, int256 y3) internal pure returns (string memory) {

        string memory svgStr;
        for (uint i = 0; i < 99; i += 3) {
            svgStr = string(
                abi.encodePacked(
                    svgStr,
                    '<polygon points="',
                    ...
                    ' fill="rgb(173,111,184)" stroke="black" stroke-width="2"></polygon>'
                )
            );            
        }
...
contract TriangleDraw {
    ...
    // int256[2][39] bodyPoints;
    int[2][39] public bodyPoints = [
        [int(702), int(404)],
        [int(667), int(440)],
        [int(631), int(475)],
        [int(596), int(510)],
    ...
    uint256 public bp_len = 39;
    int256[2][] public convex = new int256[2][](100);
    uint256 public cv_len = 0;
    int256 public min_X = 100000000;
    int256 public max_X = 0;
 
    event c_start(uint256 _c, uint256 _i );
    event open_start(int256 _dx, int256 _r);
    event edges_start(uint256 _edgeslen);
    event open_push(uint256 _edgeslen);
    ...
    function drawTriangles(uint256 point_count, int256 R, int256 G, int256 B) public view returns(string memory) {
        ...
        // int256[2][] memory value_list = new int256[2][](bp_len + point_count);
    ...
    function findConvexPoints() public returns (int256[2][] memory, uint256) {
        // int256[2][] memory convex = new int256[2][](bp_len);
        // uint256 cv_len = 0;

        for(uint i = 0; i < cv_len; i++) {
            convex[i] = [int(0), int(0)];
        }
        cv_len = 0;
        ...
        do {       
            ...
            for (uint i = 0; i < bp_len; i++)
            {
                if (Delaunay.orientation(bodyPoints[p], bodyPoints[i], bodyPoints[q]) == 2) {
                    q = i;
                }
            }
    ...
    function getYScope(int256 pt_X) public view returns(int256, int256, int256) {
        // Find the pt_y scope
        int256 min_Y = 100000000;
        int256 max_Y = 0;
        ...
        int256 pt_y = min_Y + 5 + MathLib.random(uint256(max_Y - min_Y-10), msg.sender, pt_X);

        return(min_Y, max_Y, pt_y);
    }
