...
contract TriangleDraw {
    ...
    function generatePoint(uint256 point_count) public view returns(int256[2][] memory) {
        ...
        uint256 i = 0;
        // uint256 res_len = 0;
        int256 min_x = 100000000;
        int256 max_x = 0;
        int256 min_y = 100000000;
        int256 max_y = 0;
        for(i=0; i<bodyPoints.length; i++) {
            if(bodyPoints[i][0] < min_x)
                min_x = bodyPoints[i][0];
            if(bodyPoints[i][0] > max_x)
                max_x = bodyPoints[i][0];
            if(bodyPoints[i][1] < min_y)
                min_y = bodyPoints[i][1];
            if(bodyPoints[i][1] > max_y)
                max_y = bodyPoints[i][1];
        }
        ...
        while( i < point_count) {
            int256 pt_x = 0;
            int256 pt_y = 0;
            nonce = min_x + MathLib.random(uint256(max_x-min_x), msg.sender, nonce+int(i)*int(i));
            pt_x = nonce;
            nonce = min_y + MathLib.random(uint256(max_y - min_y), msg.sender, nonce+int(i)*int(i));
            pt_y = nonce;
    ...
    function drawTriangles(uint256 point_count, int256 R, int256 G, int256 B) public view returns(string memory) {
        ...
        // int256[2][] memory value_list = new int256[2][](bodyPoints.length + point_count);
